...
class BuyerNotificationOverlay extends StatefulWidget {
  const BuyerNotificationOverlay({super.key, required this.child});
  final Widget child;

  @override
  State<BuyerNotificationOverlay> createState() =>
      _BuyerNotificationOverlayState();
}

class _BuyerNotificationOverlayState extends State<BuyerNotificationOverlay>
  ...
  void _subscribe() {
    ...
    _channel = Supabase.instance.client
        .channel('buyer-notif-overlay-${userId.substring(0, 8)}')
        .onPostgresChanges(
...
Color _buyerAccent(String type) {
  switch (type) {
    case 'order_accepted':         return const Color(0xFF2A4BA0);
    case 'order_ready':            return const Color(0xFF059669);
    case 'order_out_for_delivery': return const Color(0xFF0891B2);
    case 'order_completed':        return const Color(0xFF7C3AED);
...
Color _buyerAccentLight(String type) {
  switch (type) {
    case 'order_accepted':         return const Color(0xFF4F7FE8);
    case 'order_ready':            return const Color(0xFF10B981);
    case 'order_out_for_delivery': return const Color(0xFF06B6D4);
    case 'order_completed':        return const Color(0xFF8B5CF6);
...
IconData _buyerIcon(String type) {
  switch (type) {
    case 'order_accepted':         return Icons.restaurant_rounded;
    case 'order_ready':            return Icons.check_circle_rounded;
    case 'order_out_for_delivery': return Icons.delivery_dining_rounded;
    case 'order_completed':        return Icons.done_all_rounded;
...
bool _buyerHasAction(String type) {
  switch (type) {
    case 'order_accepted':
    case 'order_ready':
...
class _BuyerNotifCard extends StatelessWidget {
  const _BuyerNotifCard({
    required this.notif,
  ...
  Widget build(BuildContext context) {
    ...
    final title = (notif['title']?.toString().trim().isNotEmpty == true)
        ? notif['title'].toString()
        : 'Notification';
    final message = notif['message']?.toString() ?? '';
    final accent = _buyerAccent(type);
    final accentLight = _buyerAccentLight(type);
